...
from clustering_utils_mag import ClusteringResult, ClusterData
...
def prepare_data_for_clustering(filename: str) -> ClusterData:
    ...
    Returns:
        ClusterData: data ready for clustering, contains inc, raan and mag
          arrays
    """
    ...
    ecc = data[10]
    mag = data[20]
    return ClusterData(inc=inc, raan=raan, mag=mag)
...
def merge_cluster_data(cluster_data_list: list):
    ...
    return ClusterData(inc=inc_all, raan=raan_all, mag=mag_all) 
...
def bin_data_for_clustering(year_ranges: dict, print_res: bool = True): 
    ...
    for year_range, orbits in file_lists.items():
        ...
        results.append((ClusterData(inc=inc_all, raan=raan_all, mag=mag_all), year_range))
...
def bin_observed_data(uncorr_obs_files: list, year_ranges: dict, print_res: bool = False):
    ...
    for year_range, years in year_ranges.items():
        ...
        results.append((ClusterData(inc=inc, raan=raan, mag=mag), year_range))
