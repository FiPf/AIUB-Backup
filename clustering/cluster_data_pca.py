...
from clustering_utils_pca import ClusteringResult, ClusterData
...
def prepare_data_for_clustering(filename: str) -> ClusterData:
    ...
    sem_maj = data[8]
    diameter = data[1]
    return ClusterData(ecc=ecc, mag_obj=mag_obj, sem_maj=sem_maj, diameter=diameter, inc=inc, raan=raan)
...
def merge_cluster_data(cluster_data_list: list):
    ...
    return  ClusterData(ecc=ecc_all, mag_obj=mag_obj_all, sem_maj=sem_maj_all, diameter=diameter_all, inc=inc_all, raan=raan_all)
...
def bin_data_for_clustering(year_ranges: dict, print_res: bool = True):
    ...
    for year_range, orbits in file_lists.items():
        ...
        results.append((
            ClusterData(ecc=ecc_all, mag_obj=mag_obj_all, sem_maj=sem_maj_all, diameter=diameter_all, inc=inc_all, raan=raan_all),
            year_range
        ))
